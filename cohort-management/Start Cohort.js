...
function startCohort()
{
  ...
  try
  {
    ...
    // Show all sheets, otherwise the below may fail
    ss.getSheets().forEach(function (sheet) { sheet.showSheet() })
    ...
    // Protect the "Original Program Goal" column of the participants list
    protectProgramGoalColumn(participantSpreadsheet)
...
function setupCohortForms(participantData, settings)
{
  ...
  function addIndividualData(flowRow, rollupRow)
  {
    participantData.forEach(function (participant)
    {
      ...
      self.surveyLink = getPrefilledFormUrl(flowRow.surveyLink, prefillFields)
      ...
      // Get prefilled survey link with manager's email
      prefillFields['Your email address'] = manager.roleEmail
      manager.surveyLink = getPrefilledFormUrl(flowRow.surveyLink, prefillFields)
      ...
      directReportEmailsList.forEach(function (directReportEmail)
      {
        ...
        // Get prefilled survey link with direct's email
        prefillFields['Your email address'] = direct.roleEmail
        direct.surveyLink = getPrefilledFormUrl(flowRow.surveyLink, prefillFields)
...
function addToReporting(reportingData)
{
  ...
  // Individual data, which gets filtered on the sheet to "Active Individual Data"
